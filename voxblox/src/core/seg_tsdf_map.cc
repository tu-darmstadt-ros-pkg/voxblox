...
unsigned int SegmentedTsdfMap::coordPlaneSliceGetDistanceWeight(
  ...
  seg_tsdf_layer_->getAllAllocatedBlocks(&blocks);

  // Cache layer settings.
  const size_t vps = seg_tsdf_layer_->voxels_per_side();
  ...
  for (const BlockIndex& index : blocks) {
    ...
    const Block<SegmentedVoxel>& block = seg_tsdf_layer_->getBlockByIndex(index);
    ...
    for (size_t linear_index = 0; linear_index < num_voxels_per_block;
      ...
      const SegmentedVoxel& voxel = block.getVoxelByLinearIndex(linear_index);
