...
//based on seg_tsdf_integrator

namespace voxblox {

//from seg_tsdf_integrator
MrcnnTsdfIntegrator::MrcnnTsdfIntegrator(const Config& config, 
...
//from seg_tsdf_integrator
inline bool MrcnnTsdfIntegrator::isPointValid(const Point& point_C, const Label& segment) const {
    return segment > 0 && isPointValid(point_C);
}

//from seg_tsdf_integrator
inline bool MrcnnTsdfIntegrator::isPointValid(const Point& point_C) const {
...
//from seg_tsdf_integrator
void MrcnnTsdfIntegrator::updateSegmentedVoxel(const GlobalIndex& global_voxel_idx,
                                               const Label& segment) {
...
//from seg_tsdf_integrator
float MrcnnTsdfIntegrator::computeDistance(const Point& origin,
                                           const Point& point_G,
...
//from seg_to_tsdf //important TODO
void MrcnnTsdfIntegrator::integrateSegmentedPointCloud(const Transformation& T_G_C,
...
//get Visible Voxels
  for (size_t i = 0; i < config_.integrator_threads; ++i) {
    integration_threads.emplace_back(&MrcnnTsdfIntegrator::getVisibleVoxels,
                                     this, T_G_C, points_C,
                                     &index_getter);
  }
...
//from seg_to_tsdf
void MrcnnTsdfIntegrator::getVisibleVoxels(const Transformation& T_G_C,
                                                         const Pointcloud& points_C,
...
//TODO removed because missing TGC???
    //if (!success) {
    //  continue;
    //}
...
LabelIndexMap MrcnnTsdfIntegrator::propagateSegmentLabels(LabelIndexMap& segment_map) {
...
//from seg_tsdf
void MrcnnTsdfIntegrator::updateGlobalSegments(const LabelIndexMap& propagated_labels) {
...
//from seg_tsdf, handles colliding segments
float MrcnnTsdfIntegrator::computeSegmentOverlap(Label segment_1_id, Label /*segment_2_id*/, Labels& segment1_idxs, Labels& segment2_idxs) {
...
//from seg_tsdf
void MrcnnTsdfIntegrator::checkMergeCandidates(LabelIndexMap& propagated_labels) {
...
//from seg_tsdf
void MrcnnTsdfIntegrator::applyLabelToVoxels(const BlockIndex& block_idx, Label old_segment, Label new_segment) {
...
//from seg_tsdf
void MrcnnTsdfIntegrator::mergeSegmentLabels(LabelIndexMap& propagated_labels, const LabelPair& label_pair) {
