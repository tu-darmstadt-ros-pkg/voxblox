...
void VoxbloxMeshVisual::setMessage(const voxblox_msgs::Mesh::ConstPtr& msg,
  ...
  for (const voxblox_msgs::MeshBlock& mesh_block : msg->mesh_blocks) {
    ...
    Eigen::Quaternionf quat = Eigen::Quaternionf(msg->transform.rotation.w,
    ...
    for (size_t i = 0; i < connected_mesh.vertices.size(); i ++) {
      ...
      connected_mesh.vertices[i] = rot_mat * connected_mesh.vertices[i];
      connected_mesh.vertices[i](0,0) += msg->transform.translation.x;
      connected_mesh.vertices[i](1,0) += msg->transform.translation.y;
      connected_mesh.vertices[i](2,0) += msg->transform.translation.z;
    }
