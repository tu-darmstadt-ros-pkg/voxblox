#ifndef VOXBLOX_ROS_MRCNN_SERVER_H_
#define VOXBLOX_ROS_MRCNN_SERVER_H_


//include froms segmentation server
...
class MrcnnServer : public TsdfServer {
    public:
        //eigen policy?
        ...
        MrcnnServer(const ros::NodeHandle& nh, const ros::NodeHandle& nh_private);
        MrcnnServer(const ros::NodeHandle& nh, const ros::NodeHandle& nh_private, const MrcnnTsdfIntegrator::Config& mrcnn_integrator_config);
        ...
        void maskCallback(const sensor_msgs::ImageConstPtr& mask_img_msg, const sensor_msgs::ImageConstPtr& depth_img_msg, 
    ...
    void convertToCloud(const cv::Mat& depth_msg,
                        const cv::Mat& mask_msg,
                        const sensor_msgs::CameraInfoConstPtr& depth_cam_info,
                        pcl::PointCloud<pcl::PointXYZRGB>& cloud);

    void integrateSegmentation(const sensor_msgs::PointCloud2ConstPtr& pointcloud, const cv::Mat& mask_img, const cv::Mat& depth_img,
    ...
    //Subscriber
    message_filters::Subscriber<sensor_msgs::Image> image_mask_sub_;
    message_filters::Subscriber<sensor_msgs::Image> depth_image_sub_;
    message_filters::Subscriber<sensor_msgs::CameraInfo> mask_info_sub_;
    message_filters::Subscriber<sensor_msgs::CameraInfo> depth_info_sub_;
    message_filters::Synchronizer<RgbdSyncPolicy> msg_sync_;
    
    //Members
    std::shared_ptr<SegmentedTsdfMap> seg_tsdf_map_;
    std::unique_ptr<MrcnnTsdfIntegrator> mrcnn_tsdf_integrator_;
    MrcnnSegmenter mrcnn_segmenter_;
