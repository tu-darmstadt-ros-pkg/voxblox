...
int main(int argc, char** argv) {
  ...
  nh.param<std::string>("voxel_type", voxel_type, "single");

  if (voxel_type.compare("single") == 0) {
      std::cout << "voxel type single" << std::endl;
    ...
    std::shared_ptr<voxblox::SegDataFuser<voxblox::LabelVoxel>> fuser(
        new voxblox::SegDataFuser<voxblox::LabelVoxel>(nh, tsdflayer,
                                                       labellayer));
    ...
    ros::spin();
      ...
      std::cout << "voxel type multi" << std::endl;
    ...
    std::shared_ptr<voxblox::SegDataFuser<voxblox::MultiLabelVoxel>> fuser(
        new voxblox::SegDataFuser<voxblox::MultiLabelVoxel>(nh, tsdflayer,
                                                            labellayer));
    node->initDataFuser(fuser);
